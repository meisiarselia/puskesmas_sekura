        ...
        <section>
            <div class="card card-body border-0 shadow">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h4 class="m-0">Fasilitas</h4>
                    <a href="{{ route('fasilitas.create') }}" class="btn btn-success">Tambah Data</a>
                </div>
                <div class=" table-responsive">
                    <table class="table" style="width: 100%" id="myTable">
                        ...
                        <tbody>
                            @foreach ($fasilitass as $i => $fasilitas)
                                <tr>
                                    <td>{{ $i + 1 }}</td>
                                    <td>{{ $fasilitas->nama }}</td>
                                    <td>{{ $fasilitas->deskripsi }}</td>
                                    <td>
                                        <img src="{{ asset($fasilitas->gambar) }}" class="rounded" style="height: 75px">
                                    </td>
                                    <td class="aksi">
                                        <div class="d-inline-flex" style="gap: .5rem">
                                            <a href="{{ route('fasilitas.create', $fasilitas->id) }}" class="btn btn-dark">
                                                <i class="fa-solid fa-pen-to-square"></i>
                                            </a>
                                            <form id="deleteForm{{ $fasilitas->id }}"
                                                action="{{ route('fasilitas.delete', $fasilitas->id) }}" method="POST">
                                                @csrf
                                                @method('delete')
                                                <button type="button" class="btn btn-danger"
                                                    onclick="confirmDelete({{ $fasilitas->id }})">
